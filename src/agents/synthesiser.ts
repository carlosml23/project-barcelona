...
You are given:
  ...
  (5) candidates — an optional array of candidate identities detected during clustering. Each has a label (e.g. "Juan Garcia — Telefonica, Madrid"), evidence_ids, and confidence. Use these to produce per-candidate negotiation angles.
...
YOUR RULES — violating any is a disqualifying hallucination:
  ...
  • "negotiation_angles" is a map keyed by candidate label → array of 2-4 angles.
    - If candidates are provided, produce angles PER candidate keyed by their label.
    - If no candidates, use a single key "General".
    - Tailor angles to the debt context AND confidence level:
...
JSON schema:
{
  ...
  "negotiation_angles": { "Candidate Label": [string, ...], ... },  // 2-4 concrete angles PER candidate
...
export async function synthesise(
    ...
    messages: [
      {
        ...
        content: candidates && candidates.length > 0
          ? `Here is the case payload. ${candidates.length} candidate identit${candidates.length === 1 ? "y was" : "ies were"} detected — produce "negotiation_angles" as an object keyed by each candidate's label, with 2-4 angles per candidate. Return ONLY the briefing JSON.\n\n${JSON.stringify(userPayload, null, 2)}`
          : `Here is the case payload. Return ONLY the briefing JSON.\n\n${JSON.stringify(userPayload, null, 2)}`,
      },
    ],
  });
  ...
  const llmOutput = JSON.parse(jsonText);

  // Merge in fields we already know — don't rely on the LLM to echo them back
  // Normalize negotiation_angles: LLM may return old format (string[]) or omit it
  const rawAngles = llmOutput.negotiation_angles;
  let negotiationAngles: Record<string, string[]>;

  if (rawAngles && typeof rawAngles === "object" && !Array.isArray(rawAngles)) {
    negotiationAngles = rawAngles;
  } else {
    const flatAngles: string[] = Array.isArray(rawAngles) ? rawAngles : [];
    if (candidates && candidates.length > 0 && flatAngles.length > 0) {
      // LLM returned flat array despite candidates — assign all to top candidate
      negotiationAngles = { [candidates[0].label]: flatAngles };
    } else {
      negotiationAngles = { General: flatAngles };
    }
  }

  // If angles are empty but we have candidates, add a placeholder per candidate
  if (Object.values(negotiationAngles).every((a) => a.length === 0) && candidates && candidates.length > 0) {
    negotiationAngles = Object.fromEntries(
      candidates.map((c) => [
        c.label,
        [`Candidate confidence: ${Math.round(c.confidence * 100)}% — ${c.distinguishing_features[0] ?? "review evidence for angles"}`],
      ]),
    );
  }
  ...
  const parsed = BriefingSchema.parse({
    ...llmOutput,
    case_id: row.case_id,
    negotiation_angles: negotiationAngles,
    gaps: Array.isArray(llmOutput.gaps) ? llmOutput.gaps : gaps,
    overall_confidence: llmOutput.overall_confidence ?? "medium",
    generated_at: llmOutput.generated_at ?? new Date().toISOString(),
  });
  ...
  trace.push({
    ...
    agent: "synthesiser",
    kind: "decision",
    message: `findings=${validated.findings.length} angle_groups=${Object.keys(validated.negotiation_angles).length} confidence=${validated.overall_confidence}`,
  });
